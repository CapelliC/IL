...
ostream& DbVTable::Display(ostream& s, int) const {
    ...
    e_VTable *e;
    while ((e = static_cast<e_VTable *>(it.next())) != nullptr)
        s << '[' << e->db << ',' << e->first << ']';
...
ostream& DbList::Display(ostream& s, int mode) const {
    ...
    while ((e = static_cast<e_DbList *>(it.next())) != nullptr)
...
ostream& DbIntlog::Display(ostream& s, int mode) const {
    ...
    if (mode & Entries) {
        ...
        while ((e = static_cast<DbEntry*>(it.next())) != nullptr)
            e->Display(s, mode);
    }
    ...
    if (m_types.numel() > 0) {
        ...
        const DbIntlog *dbt;
        while ((dbt = static_cast<const DbIntlog *>(dbit.next())) != nullptr)

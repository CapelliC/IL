...
#define TermArgsVect(sptr)  (reinterpret_cast<Term*>(reinterpret_cast<char*>(sptr) + sizeof(Struct)))
...
vector<TermData*> Term::subterms() const {
        vector<TermData*> v;
        if (type(f_STRUCT)) {
                Struct *s = *m_structs[ivalue()];
                for (int n = get_arity() - 1; n >= 0; n--)
                        v.push_back(&(TermArgsVect(s)[n].m_data));
        }
        if (type(f_LIST)) {
                if (!LNULL()) {
                        List *l = m_lists[ivalue()];
...
TermDataVisit::TermDataVisit(TermData* t) {
    push(t);
}

TermData* TermDataVisit::next() {
    if (size()) {
        TermData *top = get();
        pop();
        /*
        auto vp = Term(*top).subterms();
        for (auto p = vp.rbegin(); p != vp.rend(); ++p)
                push(*p);
        */
        for (auto p: Term(*top).subterms())
                push(p);
        return top;
    }
    return nullptr;
...
inline TermData Term::type() const {
    return m_data & MaskFlags;
}
inline unsigned Term::ivalue() const {
    return unsigned(m_data & ~MaskFlags);
}
inline bool Term::type(TermData f) const {
    return (m_data & f) ? true : false;
}
...
string Term::show() const {
    ...
    case f_LIST:
        s << '[';
        if (!LNULL()) {
            auto x = *m_lists[ivalue()];
            s << x.head().show() << '|' << x.tail().show();
        }
        s << ']';
        break;

...
BtFImpl(os_interrupt, t, p) {
    ...
    if (!inum.type(f_INT) || !ireg.type(f_LIST)) { err:
        p->BtErr(BTERR_INVALID_ARG_TYPE);
        ireg.Destroy();
        return 0;
    }
...
BtFImpl(listfiles, t, p) {
    ...
    static ArgALIter::aDesc vect[] = { {"arch",    f_NOTERM}, {"hidden",  f_NOTERM}, {"normal",  f_NOTERM}, {"rdonly",  f_NOTERM}, {"subdir",  f_NOTERM}, {"system",  f_NOTERM}, {"volid",   f_NOTERM}
    };
...
BtFImpl(showstatus, t, p) {
    static ArgALIter::aDesc vect[] = { {"base",    f_ATOM}, {"proof",   f_ATOM}, {"vars",    f_ATOM}, {"trail",   f_ATOM}
    };
...
BtFImpl(timecurr, t, p) {
    ...
    mtime* pmt = new mtime(time(nullptr));
...
SysData::rtinfo mtime::rti = { "time", mkmtime, nullptr };

mtime* mtime::get_data(Term t) {
    if (t.type(f_SYSDATA)) {
        SysDataPtr sp = t;
        if (sp->istype(rti.id))
            return static_cast<mtime*>(sp);
    }
...
char *mtime::format(char *buf, int maxbuf, const char *fmt) const {
    ...
    if (!fmt)
        fmt = "%H:%M %d/%m/%Y";
    if (strftime(buf, size_t(maxbuf), fmt, &t) == 0)
        buf[0] = 0;
    return buf;
}
